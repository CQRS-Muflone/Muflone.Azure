...
public class ServiceBusIntegrationEventProcessorFactory<T> where T : class, IIntegrationEvent
{
		...
		IMessageMapperFactory messageMapperFactory)
	{
		...
		if (messageMapper == null)
			throw new Exception(
				$"No MessageMapper has found for {typeof(T)}. A MessageMapper must be specified for every IntegrationEvent");
		...
		IIntegrationEventHandlerFactoryAsync eventHandlerFactoryAsync)
	{
		...
		if (integrationEventHandlerAsync == null)
			throw new Exception(
				$"No EventHandler has found for {typeof(T)}. At least an EventHandler must be specified for this DomainEvent");

		var integrationEventHandlersArray = integrationEventHandlersAsync as IIntegrationEventHandlerAsync<T>[] ??
		                                    integrationEventHandlersAsync.ToArray();
		if (!integrationEventHandlersArray.Any())
			throw new Exception(
				$"No EventHandler has found for {typeof(T)}. At least an EventHandler must be specified for this DomainEvent");

		IntegrationEventProcessorAsync =
			new AzureIntegrationEventProcessorAsync<T>(brokerOptions, messageMapper, integrationEventHandlersArray);
	}
		...
		IIntegrationEventHandlerFactoryAsync eventHandlerFactoryAsync)
	{
		...
		IntegrationEventProcessorAsync =
			new AzureIntegrationEventProcessorAsync<T>(brokerOptions, integrationEventHandlersArray);
	}
}