...
public class ServiceBusEventProcessorFactory<T> where T : class, IDomainEvent
{
		...
		IDomainEventHandlerFactoryAsync eventHandlerFactoryAsync)
	{
		...
		var domainEventHandlersArray =
			domainEventHandlersAsync as IDomainEventHandlerAsync<T>[] ?? domainEventHandlersAsync.ToArray();
		...
		DomainEventProcessorAsync =
			new AzureDomainEventProcessorAsync<T>(brokerOptions, messageMapper, domainEventHandlersArray);
	}
