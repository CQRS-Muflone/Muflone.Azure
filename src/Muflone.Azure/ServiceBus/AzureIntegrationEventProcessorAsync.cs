...
public class AzureIntegrationEventProcessorAsync<T> : IIntegrationEventProcessorAsync<T>
	where T : class, IIntegrationEvent
{
		...
		IEnumerable<IIntegrationEventHandlerAsync<T>> eventHandlersAsync)
	{
		...
		_messageMapper = messageMapper;
		var integrationEventHandlerArray =
			eventHandlersAsync as IIntegrationEventHandlerAsync<T>[] ?? eventHandlersAsync.ToArray();
	...
	public void RegisterBroker()
	{
		if (!_eventHandlersAsync.Any())
			throw new Exception(
				$"No IntegrationEventHandler has found for {typeof(T)}. At least one IntegrationEventHandler must be specified for every IntegrationEvent");
